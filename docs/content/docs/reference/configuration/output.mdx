...
Enable generating factory methods for your data models. This is useful for testing or initializing empty data structures.
...
Enable or disable factory generation.
...
Prefix for the generated factory function.

### location

**Type:** `'inline-with-model' | 'separate-file' | 'combined-separate-file'`
**Default:** `'inline-with-model'`

Where to output the factory methods:
- `inline-with-model`: Appends the factory method to the end of the schema file.
- `separate-file`: Creates a new file alongside the schema file (e.g., `user.factory.ts`).
- `combined-separate-file`: Creates a single `factoryMethods.ts` file in the schema folder containing all factory methods.
...
Whether to include optional schema properties in the generated factory payload.
    ...
    output: {
      override: {
        angular: {
          ...
          runtimeValidation: true, // enable Zod schema validation
        },
      },
    },
  },
...
Controls the Angular `@Injectable({ providedIn })` scope for generated services.
...
Enable Zod runtime validation for Angular HttpClient responses. Requires `schemas: { type: 'zod' }`. When enabled, responses are validated via `Schema.parse()` in the RxJS pipeline. Only applies to JSON body responses — skipped for primitive types, `events`/`response` observe modes, and custom mutators.
